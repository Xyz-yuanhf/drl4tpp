...
if __name__ == '__main__':
    ...
    if opts.data_path is None:
        opts.data_path = 'data/random/utpp_{:d}_{:d}.pkl'.format(opts.graph_size, opts.num_products)
    ...
    for id, instance_dataloader in enumerate(dataloader):
        ...
        supply_data = torch.cat((padding, instance_dataloader['supply_data'].squeeze()), dim=0).numpy()
        price_data = torch.cat((padding, instance_dataloader['price_data'].squeeze()), dim=0).numpy() \
                     * opts.max_price
        demand = torch.ones_like(instance_dataloader['supply_data'].squeeze()[0, :]).numpy()
