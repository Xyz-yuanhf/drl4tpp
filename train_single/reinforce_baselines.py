...
import copy
from train_single.train import rollout, get_inner_model
        ...
        if dataset is None:
            if model.problem.NAME == 'utpp':
                self.dataset = self.problem.make_dataset(
                    num_samples=self.opts.val_size, graph_size=self.opts.graph_size,
                    num_products=self.opts.num_products, max_price=self.opts.max_price)
            elif model.problem.NAME == 'rtpp':
                self.dataset = self.problem.make_dataset(
                    num_samples=self.opts.val_size, graph_size=self.opts.graph_size,
                    num_products=self.opts.num_products, max_price=self.opts.max_price,
                    max_supply=self.opts.max_supply, coeff=self.opts.coeff)
        else:
