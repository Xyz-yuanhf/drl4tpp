...
import copy
from train_meta.train import rollout, get_inner_model
...
class RolloutBaseline(Baseline):
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        if dataset is not None:
            if len(dataset) != self.opts.val_size:
                ...
                dataset = None
            elif dataset[0]['loc'].size(0) != self.opts.graph_size - 1:
                print("Warning: not using saved baseline dataset since graph_size does not match")
                dataset = None
        if dataset is None:
            if model.problem.NAME == 'utpp':
                self.dataset = self.problem.make_dataset(
                    num_samples=self.opts.val_size, rand=False,
                    batch_size=self.opts.eval_batch_size, problem_config=self.opts.problem_config,
                    max_price=self.opts.max_price)
            elif model.problem.NAME == 'rtpp':
                self.dataset = self.problem.make_dataset(
                    num_samples=self.opts.val_size, rand=False,
                    batch_size=self.opts.eval_batch_size, problem_config=self.opts.problem_config,
                    max_price=self.opts.max_price, max_supply=self.opts.max_supply)
        else:
    ...
    def epoch_callback(self, model, epoch):
        ...
        print("Epoch {} candidate mean {}, baseline epoch {} mean {}, difference {}".format(
            epoch, candidate_mean * 1000, self.epoch, self.mean * 1000, candidate_mean * 1000 - self.mean * 1000))
