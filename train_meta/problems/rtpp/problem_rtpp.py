...
from train_meta.problems.rtpp.state_rtpp import StateRTPP
...
class RTPPDataset(Dataset):
    ...
    def __init__(self, filename=None, num_samples=1280, num_reuse=1, rand=True,
                 batch_size=512, problem_config=[], max_price=10, max_supply=15):
        if filename is not None:
            ...
            if not rand:
                ...
                for config in problem_config:
                    graph_size, num_products, coeff = config['graph_size'], config['num_products'], config['coeff']
                    for i in range(batch_size):
                        self.data.append(generate_rtpp_instance(graph_size, num_products,
                                                                max_price, max_supply, coeff))
            else:
                if num_reuse == 1:
                    self.data = []
                    for i in range(num_samples):
                        if i % batch_size == 0:
                            ...
                            graph_size, num_products, coeff = \
                                config['graph_size'], config['num_products'], config['coeff']
                        self.data.append(generate_rtpp_instance(graph_size, num_products,
                                                                max_price, max_supply, coeff))
                else:
                    self.data = []
                    for i in range(num_samples // num_reuse):
                        if i % batch_size == 0:
                            config = random.choice(problem_config)
                            graph_size, num_products, coeff = \
                                config['graph_size'], config['num_products'], config['coeff']
